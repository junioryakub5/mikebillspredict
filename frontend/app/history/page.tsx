...
export default function HistoryPage() {
  ...
  return (
    <>
      <Navbar />
      <main className="min-h-screen pt-24 pb-16 relative z-10" style={{ background: "var(--bg-card)" }}>
        <div className="page-container">
          {/* Header */}
          <div className="mb-8">
            <h1 className="font-brand text-3xl font-bold mb-2" style={{ color: "var(--text-primary)" }}>Past Results</h1>
            <p style={{ color: "var(--text-secondary)" }}>Our winning history and proven track record</p>
          </div>
          ...
          {!loading && predictions.length > 0 && (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-bold text-lg" style={{ color: "var(--text-primary)" }}>Proven Success</h2>
                  <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
                    Join thousands who trust Mike Bills Predict
                  </p>
                </div>
              </div>
              <div className="flex items-center gap-8 md:ml-auto">
                ...
                <div className="text-center">
                  <p className="text-2xl font-black" style={{ color: "#6366f1" }}>{predictions.length}</p>
                ...
                style={
                  filter === f
                    ? f === "win"
                      ...
                      : { background: "#6366f1", color: "#ffffff", borderColor: "#6366f1", boxShadow: "0 4px 12px rgba(99,102,241,0.2)" }
          ...
          {loading ? (
            ...
            <div className="text-center py-24">
              ...
              <p className="text-lg mb-2" style={{ color: "var(--text-secondary)" }}>No results yet</p>
  ...
  return (
    <div className="card-glass overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-3 px-4 md:px-6 pt-4 md:pt-5 pb-4">
        <h3 className="font-bold text-base md:text-lg" style={{ color: "var(--text-primary)" }}>{prediction.match}</h3>
        <span
          className="text-xs font-bold px-3 py-1 rounded-lg"
          style={{ background: "rgba(99,102,241,0.06)", color: "#6366f1", border: "1px solid rgba(99,102,241,0.12)" }}
        >
        ...
        <span className="text-sm ml-auto flex items-center gap-1" style={{ color: "var(--text-muted)" }}>
          <Calendar size={13} />
      ...
      {hasImages ? (
        <div className={`grid gap-0 ${hasBefore && hasProof ? "grid-cols-2" : "grid-cols-1"}`}
          ...
          {hasBefore && (
            <div className={`flex flex-col ${hasBefore && hasProof ? "border-r" : ""}`} style={{ borderColor: "var(--border)" }}>
              <div className="flex items-center justify-between px-3 py-2"
                style={{ background: "var(--bg-base)", borderBottom: "1px solid rgba(0,0,0,0.06)" }}>
                <span className="text-[11px] font-bold uppercase tracking-wide" style={{ color: "var(--text-secondary)" }}>Bet Slip</span>
                <span className="text-[10px] uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>Before</span>
              </div>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={prediction.imageUrl!} alt={`Bet slip — ${prediction.match}`}
                className="w-full object-contain"
                style={{ background: "var(--bg-surface)", maxHeight: hasBefore && hasProof ? "280px" : "360px", minHeight: "140px" }}
              />
            </div>
          )}
          {hasProof && (
            <div className="flex flex-col">
              <div className="flex items-center justify-between px-3 py-2"
                ...
                <span className="text-[10px] uppercase tracking-widest" style={{ color: "var(--text-muted)" }}>After</span>
              </div>
              {/* eslint-disable-next-line @next/next/no-img-element */}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 px-4 md:px-6 pb-5"
          ...
          <div>
            <p className="text-xs font-semibold text-center mb-3" style={{ color: "var(--text-secondary)" }}>Our Prediction</p>
            <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--border)" }}>
              <div className="px-4 py-3 flex justify-between items-center"
                style={{ background: "var(--brand)" }}>
                <span className="font-black text-sm tracking-wider text-white">PREDICTION</span>
              ...
              <div className="p-4" style={{ background: "var(--bg-base)" }}>
                <p className="text-center text-xs mb-1" style={{ color: "var(--text-muted)" }}>Booking Code</p>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-bold" style={{ color: "#6366f1" }}>{prediction.odds}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span style={{ color: "var(--text-muted)" }}>League</span>
                    <span className="font-medium" style={{ color: "var(--text-secondary)" }}>{prediction.league}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="rounded-xl h-full flex flex-col items-center justify-center py-8 px-4 text-center"
              ...
              <p className="text-sm" style={{ color: "var(--text-muted)" }}>{prediction.match}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between px-4 md:px-6 py-3"
        ...
        <span className="text-xs" style={{ color: "var(--text-muted)" }}>{prediction.league}</span>
